...
if (_tugslot == tsCenter) then {_attachDir = -180; _attachDist = 40;};
if (_tugslot == tsRight) then {_attachDir = -210; _attachDist = 40;};
if (_tugslot == tsLeft) then {_attachDir = -150; _attachDist = 40;};
...
while {!isNull(((vehicleAttached select _tugindex) select tsTugged) select _tugslot) && (alive _boat) && (alive _vehicle)}do
{
	_posBoat = getPos _boat;
	_dirBoat = getDir _boat;
	...
	_velBoat = velocity _boat;
	_velVehicle = velocity _vehicle;
	_speedDiffHorz = ([[_velVehicle select 0, _velVehicle select 1], [_velBoat select 0, _velBoat select 1]] call fVectorSubstract) call fVectorLength;
	...
	if (_dirDiff < -1) then {_vehicle setDir (1 + getDir _vehicle);};
	_vehicle setVelocity ([[velocity _boat, 0.8] call fVectorScale, _vVelAdd] call fVectorAdd);
};
